...
def build_catalyst_prompt(row: Dict[str, Any], side: str) -> str:
    ...
    return (
        ...
        "Give a 2-3 sentence rationale grounded ONLY in the row, then output the "
...
def _verdict_block(c: Dict[str, Any]) -> str:
    ...
    return (
        f"{_row_line(row)}\n"
        f"  side={c.get('side')} triage_verdict={c.get('verdict')}\n"
...
def generate_note(
    ...
    Returns {"markdown", "candidates", "asof", "provider", "error"}. With
    provider=None / unavailable returns the candidate selection with
    markdown="" and a clear "set a key" error — NEVER raises. ``with_news`` is a
    passthrough flag (only meaningful for Perplexity): when True we append an
    instruction to use live recent-news context; no new HTTP client is added.
    """
    ...
    for side, key in (("long", "longs"), ("short", "shorts")):
        for r in sel[key]:
            verdict, rationale = "NEEDS_DATA", ""
            try:
                text = provider.complete(build_catalyst_prompt(r, side), max_tokens=300)
            ...
            triaged[key].append({
                "row": r, "side": side, "verdict": verdict, "rationale": rationale,
            })
    ...
    # Attach the triage verdict to the candidate summary for display/persistence.
    by_ticker = {}
    for key in ("longs", "shorts"):
        for c in triaged[key]:
            by_ticker[(c["side"], c["row"].get("ticker"))] = c["verdict"]
    for c in candidates:
        c["verdict"] = by_ticker.get((c["side"], c["ticker"]))
