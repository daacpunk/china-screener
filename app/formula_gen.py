...
def build_formula_workbook(
    ...
    if method.upper() == "A" and layout == "spill":
        instr += [
            ...
            ["---------- COPY THIS MACRO ----------"],
            ["Sub ActivateSpills()"],
            ["  ' Re-enters EVERY formula-text cell in row 2 of each ticker sheet"],
            ["  ' so it activates/refreshes: B/C/D spill arrays AND the single-value"],
            ["  ' columns E/F (ex-dividend date, company name). Scans all used"],
            ["  ' columns instead of a fixed list, so added columns always refresh."],
            ["  Dim ws As Worksheet, c As Range, col As Long, lastCol As Long"],
            ["  For Each ws In ThisWorkbook.Worksheets"],
            ['    If ws.Name <> "Instructions" And ws.Name <> "Manifest" Then'],
            ["      lastCol = ws.Cells(2, ws.Columns.Count).End(xlToLeft).Column"],
            ["      For col = 2 To lastCol   ' skip col A (ticker literal)"],
            ["        Set c = ws.Cells(2, col)"],
            ['        If Left(c.Text, 1) = "=" Then'],
            ["          Dim f As String: f = c.Text"],
            ['          c.Value = ""            ' + "' clear the inert text"],
            ["          c.Formula2 = f          " + "' enter as formula (spills if array)"],
            ["        End If"],
            ["      Next col"],
            ["    End If"],
            ["  Next ws"],
