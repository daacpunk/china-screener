...
def test_build_workbook_method_a_opens():
    ...
    assert [c.value for c in ws[1]] == [
        "close", "volume", "ex_dividend_date", "company_name"]
...
def test_build_workbook_method_a_with_date_column():
    ...
    ws = wb["9988-HK"]
    assert [c.value for c in ws[1]] == [
        "date", "close", "volume", "ex_dividend_date", "company_name"]
...
def test_build_workbook_method_a_stacked():
    ...
    assert [c.value for c in ws[1]] == [
        "ticker", "close", "volume", "ex_dividend_date", "company_name"]
...
def test_build_workbook_spill_layout_bcd_columns():
    ...
    assert "9988-HK" in wb.sheetnames
    ws = wb["9988-HK"]
    # Event-date columns land AFTER the close/volume block, so cols B/C/D
    # (date/close/volume) remain untouched for the spill-activation macro.
    assert [c.value for c in ws[1]] == [
        "ticker", "date", "close", "volume", "ex_dividend_date", "company_name"]
